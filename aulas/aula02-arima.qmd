...
2. **Decomposição STL** — confirme o período e estime a força sazonal $F_s$. Se $F_s > 0{,}64$, a sazonalidade é forte o suficiente para justificar $D = 1$; se $F_s$ é baixa, sazonalidade pode ser melhor tratada como **determinística** (dummies, Fourier).
3. **Subseries plot / boxplot por mês (ou unidade do ciclo)** — verifique se a amplitude sazonal é estável (sugere sazonalidade aditiva) ou cresce com o nível (sugere log/Box-Cox antes de modelar).
...
5. **ACF/PACF de $\Delta_s Y_t$** — depois da diferença sazonal, reavalie se ainda há tendência (decaimento lento na ACF não-sazonal). Se sim, aplique também $\Delta$ regular.

Só então identifique $(p, q, P, Q)$ na série duplamente diferenciada $\Delta\Delta_s Y_t$ — essa é a ordem que o `AutoARIMA` também segue.
:::
