...
(Amazon) é o mais fácil: open-source, roda na CPU, sem chave de API. Para
rodar nas mesmas janelas, precisamos organizar o código na mão, mas é tranquilo.

```{python}
# Chronos (Amazon) - pip install chronos-forecasting
import torch
...
n, H, W = len(df), 12, 4
test_starts = [n - (W - i) * H for i in range(W)]   # mesmos cortes da CV

linhas = []
for start in test_starts:
    ctx = torch.tensor(df["y"].iloc[:start].values, dtype=torch.float32)
    _, mean = pipe.predict_quantiles(               # zero-shot: nenhum treino
        ctx, prediction_length=H, quantile_levels=[0.5]
    )
    linhas.append(pd.DataFrame({
        "ds": df.index[start:start + H],
        "chronos": mean.numpy().flatten()[:H],
    }))
cv_chronos = pd.concat(linhas, ignore_index=True)
cv_chronos.head(3)
```
